...
| L11 | ฟอร์ม Contact: เปิดได้เมื่อมีครบ validate ฝั่ง server · rate limit · error ไม่หลุด · ข้อความแจ้งการใช้ข้อมูล · ช่องทางแจ้งเจ้าของ | OpenCode + Claude | P2 | หลัง v1 | D3 · API พร้อมแล้ว (Lab 05) แต่ยังไม่เปิด UI ตาม D3 · อยู่ใน Out of scope v1 |
| L12 | ทดสอบ 429 rate limit จริงด้วย Playwright/manual (ยังทดสอบแค่ unit/contract) | OpenCode + Claude | P2 | Lab 06 QA | rate limiter เป็น in-memory ต่อ process — v1 พอสำหรับ single instance |
